...
$(document).ready(function() {
    ...
    '<li class="fileuploader-input"><button type="button" class="fileuploader-input-inner"><i class="fileuploader-icon-main ti ti-cloud-upload"></i> <span>${captions.feedback}</span></button></li>' +
    '</ul>' +
    '</div>',
    item: '<li class="fileuploader-item">' +
    '<div class="fileuploader-item-inner">' +
      '<div class="actions-holder">' +
        '<button type="button" class="fileuploader-action fileuploader-action-sort is-hidden" title="${captions.sort}"><i class="fileuploader-icon-sort ti ti-arrows-move"></i></button>' +
        /*'<button type="button" class="fileuploader-action fileuploader-action-settings is-hidden" title="${captions.edit}"><i class="fileuploader-icon-settings ti ti-settings"></i></button>' +*/
        '<button type="button" class="fileuploader-action fileuploader-action-remove" title="${captions.remove}"><i class="fileuploader-icon-remove ti ti-x"></i></button>' +
    ...
    '<div class="fileuploader-item-inner">' +
      '<div class="actions-holder">' +
        '<button type="button" class="fileuploader-action fileuploader-action-sort" title="${captions.sort}"><i class="fileuploader-icon-sort ti ti-arrows-move"></i></button>' +
        /*'<button type="button" class="fileuploader-action fileuploader-action-settings" title="${captions.edit}"><i class="fileuploader-icon-settings ti ti-settings"></i></button>' +*/
        '<button type="button" class="fileuploader-action fileuploader-action-remove" title="${captions.remove}"><i class="fileuploader-icon-remove ti ti-x"></i></button>' +
        '<div class="gallery-item-dropdown">' +
    ...
    '</li>',
		...
		upload: {
			url: '../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=upload',
			data: null,
			type: 'POST',
		...
		editor: {
			...
			onSave: function(dataURL, item) {
				...
				if (item.appended && item.data.listProps) {
					...
					$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=resize', {name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
						// update the image
		...
		sorter: {
			onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
				...
				// send request
				$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=sort', {
			...
			$('body').on('click', function(e) {
				...
				if ($target.is('.gallery-action-rename')) {
					...
					if (x && item.data.listProps) {
						$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=rename', {name: item.name, id: item.data.listProps.id, title: x}, function(result) {
							try {
				...
				if ($target.is('.gallery-action-asmain') && item.data.listProps) {
					$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=asmain', {name: item.name, id: item.data.listProps.id}, function() {
		...
		onRemove: function(item) {
			// send request
			if (item.data.listProps)
				$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=remove', {
					name: item.name,
					id: item.data.listProps.id
				});
		},
	...
	// preload the files
	$.post('../controller/gallery.php?product=<?= $productDetails['product_id']; ?>&type=preload', null, function(result) {
