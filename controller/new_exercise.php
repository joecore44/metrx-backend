...
if(check_permission('create_exercises')){
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if(empty($errors)){
		...
		$statment = connect()->prepare("INSERT INTO exercises (
			exercise_author,
			exercise_title,
		...
		$statment->execute(array(
			':exercise_author' => $memberInfo['member_id'],
			...
			':exercise_bodyparts' => (isset($_POST['exercise_bodyparts']) && !empty($_POST['exercise_bodyparts']) ? json_encode($_POST['exercise_bodyparts']) : ""),
			':exercise_equipments' => (isset($_POST['exercise_equipments']) && !empty($_POST['exercise_equipments']) ? json_encode($_POST['exercise_equipments']) : ""),
			':exercise_levels' => (isset($_POST['exercise_levels']) && !empty($_POST['exercise_levels']) ? json_encode($_POST['exercise_levels']) : ""),
			':exercise_instructions' => $exercise_instructions,
			':exercise_image' => $uploadedImages['exercise_image']
		));
