...
$id_recipe = cleardata(getId());

if(!$id_recipe){
	header('Location: home.php');
}

if(check_permission('view_recipes') || check_permission('edit_recipes')){

	if($_SERVER['REQUEST_METHOD'] == 'POST'){

	if(check_permission('edit_recipes')){
	...
	$recipe_categories = json_encode($_POST['recipe_categories']);
	...
	$statment = connect()->prepare("UPDATE recipes SET
	...
	':recipe_description' => $recipe_description,
	':recipe_status' => $recipe_status,
	':recipe_categories' => (isset($_POST['recipe_categories']) && !empty($_POST['recipe_categories']) ? $recipe_categories : ""),
	':recipe_ingredients' => $recipe_ingredients,
	':recipe_steps' => $recipe_steps,
	':recipe_availability' => $recipe_availability,
	':recipe_featured' => $recipe_featured,
	':recipe_image' => (isset($uploadedImages['recipe_image']) ? $uploadedImages['recipe_image'] : $_POST['recipe_image_save'])
	));
...
$recipeDetails = get_recipe_per_id($id_recipe);
